...
var calen = function(params) {
    ...
    var real_id_name = params.real_id_name || ''; //selector id to populate full date object
    ...
    if (params.min_date == 'Invalid Date') {
        var min_date = get_min_date('');
    } else {
        var min_date = get_min_date((params.min_date || 0)); // min date user can scroll back to
    }
    var showToday = params.showToday || false;
    if (document.getElementById(id_name).getAttribute('data-date') != "") {
        var req_date = document.getElementById(id_name).getAttribute('data-date');
        var temp_req_date = req_date.split('/');
        var currdate = new Date(temp_req_date[2],temp_req_date[0]-1,temp_req_date[1]);
    } else {
    ...
    var list_of_holidays = params.list_of_holidays || '';
    go_cal[id_name] = {};
    var citem = {
        day: 0,
        month: 0,
...
/*        resetVals: function() {
            if (document.getElementById(id_name).getAttribute('data-date') != "") {
                var req_date = document.getElementById(id_name).getAttribute('data-date');
                var temp_req_date = req_date.split('/');
                citem.year = parseInt(temp_req_date[2]);
                citem.month = parseInt(temp_req_date[0]-1);
                asd = tru
                var currdate = new Date(temp_req_date[2],temp_req_date[0]-1,temp_req_date[1]);
            } else {
                var currdate = new Date();
    ...
    var select_date = function(mm, dd, yy) {
        ...
        if (id_name != '') {
            if (locale === 'en') {
                ...
                var tempDateStr = dateStr.toDateString();
                selectedDateStr = tempDateStr.split(' ')[0]+',' +' ' +tempDateStr.split(' ')[2]+' ' + tempDateStr.split(' ')[1];
                if (id_name == 'start-date') {
                    go_cal.onwardDate = dateStr;
                } 
                else if (id_name == 'end-date'){
                    go_cal.returnDate = dateStr;
                }
                else {
                    go_cal[id_name] = dateStr;
                }
                document.getElementById(id_name).value = selectedDateStr;
                try {
                    document.getElementById(real_id_name).value = dateStr;
                } catch(e) {};
            }
        }
        ...
        if (document.getElementById(dp_id_name).innerHTML != '') {
            close_datepicker();
        }
    };
    /**
    ...
    var month_inc = function() {
        if (mn !=0 && mn == 1 && today.getDate() > 28) {
            var scroll_date = new Date(yy, mn+1, today.getDate());
        } else {
    ...
    var month_dec = function() {
        if (today.getDate() == 31 && mn != 1) {
            var scroll_date = new Date(yy, mn, today.getDate()-1);
        } 
        else if (mn == 1 && today.getDate() > 28) {
            var scroll_date = new Date(yy, mn, today.getDate()-3);
        }
        else {
            var scroll_date = new Date(yy, mn, today.getDate());
        }
    ...
    var that = {
        ...
        set_date: function(mm,dd,yy) {
            select_date(mm,dd,yy);
            this.pop_current_date('false');
        },
        pop_current_date: function(state) {
            var selected_date = document.getElementById(id_name).getAttribute('data-date');
            if (id_name && selected_date != '') {
                selected_date = selected_date.split('/');
                yy = selected_date[2];
                mn = selected_date[0] - 1;
                that.show();
                if (state.toLowerCase() == 'false') {that.hide();}
            }
        },
        onClose: function() {
            var onclose_callback = (params.onClose instanceof Function) ? params.onClose : null;
        },
        DatePicked: function (trigger, datepicker, mindays, maxrange,start_id,end_id) {
            if(start_id && end_id) {
                var checkin_date = (document.getElementById(start_id)) ? document.getElementById(start_id).getAttribute('data-date') : null;
                var checkout_date = (document.getElementById(end_id)) ? document.getElementById(end_id).getAttribute('data-date') : null;
            }
            if(trigger == 'start-date' || trigger == 'end-date') {
            ...
            if (cindate != null) {
                ...
                if (datepicker && maxrange) {
                    var lastCinDay = days_in_month(cindate.getMonth(), cindate.getFullYear());
                    var maxdate = new Date(max_checkout_date.getFullYear(), max_checkout_date.getMonth(), max_checkout_date.getDate());
                    if (cindate.getDate() == lastCinDay || Math.abs(cindate.getDate()-lastCinDay) == 1) {
                        datepicker.set_max_date(maxdate, maxdate);
                    } else {
        ...
        show: function() {
            ...
            for (i = 0; i < display_count; i++) {
                citem.day = 1;
                citem.month = mn;
                citem.year = parseInt(yy); 
                if (i > 0) {
                    if (mn < 11) {
                        citem.month = mn + 1;
                    } else {
                        citem.month = 0;
                        citem.year = parseInt(parseInt(yy)+1)
                    }
                }
                ...
                if (!type_of_calen) {
                    var checkLimit = new Date(); checkLimit = checkLimit.getMonth();
                    calendar_html += '    <tr class="jrdp_calendar_month_tbar' + citem.multi_cal + '">';
                    if (parseInt(checkLimit+1) == citem.month) {
                        calendar_html += '            <td colspan="1" class="jrdp_calendar_month_prev' + citem.multi_cal + '" align="left">';
                    } else {
                        calendar_html += '            <td colspan="1" class="limitPrevious jrdp_calendar_month_prev' + citem.multi_cal + '" align="left">';
                    }
                    calendar_html += '                <span class="width100 fl" id="' + unique_id + 'prevmonth' + citem.multi_cal + '_' + i + '">&lsaquo;</span></td>';
                    if (showToday) {
                    ...
                    if ((max_date.getFullYear()).toString()+(max_date.getMonth()).toString() == ((citem.year).toString()+(citem.month).toString())) {
                        calendar_html += '            <td colspan="1" class="limitNext jrdp_calendar_month_next' + citem.multi_cal + '" align="right">';
                    } else {
                        calendar_html += '            <td colspan="1" class="jrdp_calendar_month_next' + citem.multi_cal + '" align="right">';
                    }
                    calendar_html += '                <span class="width100 fl" id="' + unique_id + 'nextmonth' + citem.multi_cal + '_' + i + '">&rsaquo;</span></td>';
                    calendar_html += '    </tr>';
                } else {
                    calendar_html += '    <tr class="jrdp_calendar_month_tbar' + citem.multi_cal + '">';
...
/*            $('#'+dp_id_name).css('bottom','');
            if (parseInt($(window).height() - ($('#'+id_name).offset().top-$('#start-date').height())) < $('#'+dp_id_name).height()) {
                $('#'+dp_id_name).css('bottom','-95px');
            }*/
            ...
            if (!type_of_calen && document.getElementById('show_today')) {
...
/*            $('#'+id_name).on('click', function() {
                that.pop_current_date('true');
            });*/

            if (list_of_holidays) {
                var get_list = list_of_holidays;
                for (i in get_list) {
                    var holiday_classes = document.getElementById('jrdp_start-calen_'+i);
                    if (holiday_classes) {
                        holiday_classes.className += ' dummy_holidays';
                    }
                } 
            }
        }
